...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2116
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1402
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 714
```
