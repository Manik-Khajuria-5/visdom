...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2332
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1569
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 763
```
