...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2125
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1405
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 720
```
